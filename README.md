...
temperatures, local device health, fan state, and investment statistics from
the companion XTB Home Assistant integration.
...
- Home Assistant imported sensors/text sensors for Tomorrow.io weather, local
  room conditions, fan state, and compact XTB DeskHub attributes.
...
- Bluetooth proxy removed from the default firmware to reduce power draw.
- Optional battery sleep mode with timer wakeup and GPIO21 button wakeup.
...
limit in large local builds. The configuration reads
...
If Windows still reports `The command line is too long`, build from the Home
Assistant ESPHome add-on, Docker, WSL, or Linux. For local display/touch/UI
bring-up on Windows, the lite variant is still available:
...
substitutions:
  ...
  entity_xtb_deskhub: sensor.xtb_53600874_deskhub
```

The investment panel prefers the compact DeskHub sensor generated by the XTB
Home Assistant integration. The firmware subscribes only to its compact
`summary` and `rows` attributes instead of tracking every XTB position as a
separate Home Assistant entity. It also keeps a 24-point local chart history in
ESPHome preferences, so the profit line does not restart from a flat placeholder
after firmware updates or device restarts.

## Battery Mode

The default firmware keeps the desk UI awake so touch, scrolling, OTA and Home
Assistant service calls remain available. For battery use, enable
`LilyGO T5 DeskHub Battery Sleep Mode` in Home Assistant or press
...
- Bluetooth proxy is intentionally not included in the default firmware because
  it keeps the radio busy and materially hurts battery life.
...
- Battery charging on the LILYGO Screen-4.7-S3 V2.4 is handled by the board
  hardware. The schematic shows an HX6610S charger connected to VBUS and BAT
  with CHRG/STDBY LED outputs; there is no ESP32 GPIO or I2C PMU setup required
  to start charging.
